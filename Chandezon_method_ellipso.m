clearvars -except permW; %clc;
%clear all;
%% Set constants
tic %start counting computing time
%
load('nickel.mat');
load('nickel_ellipso.mat');
Spectrum=zeros(2,100);
Spectrum2=zeros(2,100);
RefSpec0=zeros(2,100);
d=917;                  %period of the grating
kk=1;
h=200.4;                %depth of the grating
K=2*pi/d;               
...
mu2=-n2*n2/eps2;        %relative permeability of transmitted medium
end
...
%Sawtooth profile
%a_fun=@(x) 0.6*(sawtooth(-2*(K/k0).*x).*(x<=0)+sawtooth(2*(K/k0).*x).*(x>0))-cos(x);
%a_diff_fun=@(x) 0.6*(-2.*(x<=0)+2.*(x>0))+sin(x);
...
%% Auxiliary constants
m1=-20;                  %lower truncation order
m2=20;                   %upper truncation order
tol2=10e-10;             %error tolerance
tol=10e-10;
...
%% Sort eigenvalues
real_eig1p_ind=(abs(imag(rho1))<=tol2)&(real(rho1)>tol2);    %indices of eigenvalues with positive real part, incident medium
real_eig2n_ind=(abs(imag(rho2))<=tol2)&(real(rho2)<-tol2);    %indices of eigenvalues with negative real part, transmission medium
...
imag_eig1p_ind=(abs(imag(rho1))>tol)&(real(rho1)+imag(rho1)>tol);
imag_eig2n_ind=(abs(imag(rho2))>tol)&(real(rho2)+imag(rho2)<tol);
...
RefSpec0(jj,ii)=etaR(real_Ray1_idx==0);

Spectrum(jj,ii)=RVec(-min(real_Ray1_idx)+1);
Spectrum2(jj,ii)=etaR(real_Ray1_idx==0);
end
end
...
figure
subplot(1,2,1)
plot(Lambda(3:62),psi(1,3:62),'r',NiEllipso30(:,1),NiEllipso30(:,2),'.k')
title(strcat('\vartheta= ',num2str(thI/pi*180),'°'))
xlabel('wavelength [nm]')
ylabel('\psi [deg]')

subplot(1,2,2)
plot(Lambda(2:62),-delta(1,2:62),'r',NiEllipso30(:,1),NiEllipso30(:,3),'.k')
title(strcat('\vartheta= ',num2str(thI/pi*180),'°'))
xlabel('wavelength [nm]')
ylabel('\Delta [deg]')
