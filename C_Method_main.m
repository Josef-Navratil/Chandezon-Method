...
Spectrum=zeros(2,100);                  % preallocate field for the optical response parameters
...
switch output_plot  %switch between diffraction efficiencies and ellipsometric parameters
 case 'DEF'  %case 0th order diffraction efficiency
    ...
    [RVec, real_Ray1_idx, real_Ray2_idx,m1, m2, B1, B2, b0]=Make_Computation( Lam, eps1, eps2, 0 );   
%% Print Efficiencies
        nDim=m2-m1+1;
        ...
        Spectrum(1,i)=etaR(RAmp);
        if imag(n2)==0
            Spectrum(2,i)=etaT(TAmp);
        end
    end
    if(imag(n2)~=0)
    figure(2)
 ...
 case 'EMP'
   ...
   for jj=1:2
    ...
    figure(2)
    subplot(1,2,1)
    plot(VecWaveLength,psi,'r')
    title(strcat('Incident angle \vartheta= ',num2str(thI/pi*180),'°'))
    xlabel('wavelength [m]')
    ylabel('\psi [deg]')

    subplot(1,2,2)
    plot(VecWaveLength,delta,'r')
...
if strcmp(Switches(1).PlotFields,'YES') 
    ...
    [RVec, real_Ray1_idx, real_Ray2_idx,m1, m2, B1, B2, b0]=Make_Computation( Lam, eps1, eps2, 1 ); 
end
